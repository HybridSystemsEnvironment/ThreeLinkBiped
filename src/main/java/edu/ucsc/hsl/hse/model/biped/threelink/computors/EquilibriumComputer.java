...
public class EquilibriumComputer
{
	...
	private Double lT;
	private Double t;
	private BipedState biped;
	private BipedParameters params;

	public EquilibriumComputer(BipedState biped_state, BipedParameters params)
	{
		biped = TrajectoryParameters.getBipedState(biped_state);
		this.params = params;
		mL = params.legMass;
		mT = params.torsoMass;
		mH = params.hipMass;
	...
	{
		BipedState initial = ObjectCloner.deepInstanceClone(biped);
	...
	{
		BipedState finalState = ObjectCloner.deepInstanceClone(biped);// .component().copy(false, true);
	...
	{
		return (2 * lL * sA
		* (8 * sq(mH) * cos(sA - tA) - 8 * sq(mH) * cos(3 * sA + tA) + 3 * sq(mL) * cos(sA - tA)
		...
		/ (lT * t
		* (8 * mH * mL + 16 * mH * mT + 4 * mL * mT + 32 * sq(mH) * cos(2 * sA) - 4 * sq(mL) * cos(4 * sA)
		+ 2 * sq(mL) * cos(8 * sA) + 12 * sq(mT) * cos(2 * sA) - 12 * sq(mT) * cos(2 * tA)
		- 8 * sq(mT) * cos(2 * sA + 2 * tA) + 4 * sq(mT) * cos(2 * sA + 4 * tA) - 4 * sq(mT) * cos(4 * sA + 2 * tA)
		+ 2 * sq(mT) * cos(4 * sA + 4 * tA) + 16 * sq(mH) - sq(mL) + 6 * sq(mT) + 16 * mH * mL * cos(2 * sA)
		- 16 * mH * mL * cos(4 * sA) - 8 * mH * mL * cos(6 * sA) + 32 * mH * mT * cos(2 * sA)
		+ 8 * mL * mT * cos(2 * sA) - 8 * mL * mT * cos(4 * sA) - 4 * mL * mT * cos(6 * sA) - 24 * mH * mT * cos(2 * tA)
		- 12 * mL * mT * cos(2 * tA) - 16 * mH * mT * cos(2 * sA + 2 * tA) - 8 * mH * mT * cos(4 * sA + 2 * tA)
		+ 4 * mL * mT * cos(2 * sA - 2 * tA) - 4 * mL * mT * cos(2 * sA + 2 * tA) + 4 * mL * mT * cos(4 * sA - 2 * tA)
		+ 4 * mL * mT * cos(4 * sA + 2 * tA) + 4 * mL * mT * cos(6 * sA + 2 * tA)));
	...
	{
		return (6 * sA
		* (16 * mH * sq(sin(sA)) - 11 * mL - 20 * mH + 8 * mL * sq(sin(sA)) - 4 * mT * sq(sin(sA))
		- 20 * mT * sq(sin(tA)) + 16 * mH * sq(sin(2 * sA)) + 12 * mL * sq(sin(2 * sA)) + 8 * mT * sq(sin(2 * sA))
		+ 2 * mT * sin(2 * sA) * sin(2 * tA) + 24 * mT * sq(sin(sA)) * sq(sin(tA))))
		/ (t * (12 * mH - 3 * mL - 16 * mH * sq(sin(sA)) - 8 * mT * sq(sin(sA)) + 8 * mT * sq(sin(tA))
		+ 4 * mL * sq(sin(2 * sA)) + 4 * mT * sq(sin(sA + tA))));
	}
	...
	{
		return -(24 * sA
		* (8 * mH * sq(sin(sA)) - 9 * mL - 12 * mH + 6 * mL * sq(sin(sA)) - 4 * mT * sq(sin(sA)) - 12 * mT * sq(sin(tA))
		+ 12 * mH * sq(sin(2 * sA)) + 10 * mL * sq(sin(2 * sA)) + 6 * mT * sq(sin(2 * sA))
		+ 2 * mT * sin(2 * sA) * sin(2 * tA) + 16 * mT * sq(sin(sA)) * sq(sin(tA))))
		/ (cu(t) * (12 * mH - 3 * mL - 16 * mH * sq(sin(sA)) - 8 * mT * sq(sin(sA)) + 8 * mT * sq(sin(tA))
		+ 4 * mL * sq(sin(2 * sA)) + 4 * mT * sq(sin(sA + tA))));
	}
	 ...
	 * 0.600000000000000 0.200000000000000 // 0.00879611678413795
	 * -1.09178336809296 1.00534223184332 // Last : 0.600000000000000
	 * -0.600000000000000 0.200000000000000 // 1.09815805034499 1.06781645227266
	 * 0 Agent bipedAgent = new Agent(new
	 * AgentStateData(ThreeLinkThreeLinkBipedStateElements.values()));

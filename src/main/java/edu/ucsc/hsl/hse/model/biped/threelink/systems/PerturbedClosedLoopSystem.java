...
public class PerturbedClosedLoopSystem extends HybridSystem<PerturbedClosedLoopBipedState> {
	...
	public BipedDynamicsController realControl;

	public PerturbedClosedLoopSystem(PerturbedClosedLoopBipedState state, VirtualBipedState reference,
			BipedParameters params, BipedVirtualControl virtual_control, BipedDynamicsController real_control,
	...
	private void prepare() {
		...
		getComponents().getState().virtualBiped.trajParams = controller.recomputeTrajectoryParams(
				getComponents().getState().virtualBiped, getComponents().getState().virtualBiped.trajTimer);
